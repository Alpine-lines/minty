...
/**
 * @title BundleRandomness
 * BundleRandomness- support for a randomized and openable lootbox.
 */
library BundleRandomness {
  using SafeMath for uint256;

  // Event for logging lootbox opens
  event BundleOpened(uint256 indexed optionId, address indexed buyer, uint256 boxesPurchased, uint256 itemsMinted);
   ...
   */
  function initState(
    BundleRandomnessState storage _state,
   ...
   * This is called via safeTransferFrom when CreatureAccessoryBundle extends
   * CreatureAccessoryFactory.
  ...
  function _mint(
    ...
    require(settings.maxQuantityPerOpen > 0, "BundleRandomness#_mint: OPTION_NOT_ALLOWED");
    ...
    // Event emissions
    emit BundleOpened(_optionId, _toAddress, _amount, totalMinted);
  }
  ...
  function _pickRandomAvailableTokenIdForClass(
    ...
    revert("BundleRandomness#_pickRandomAvailableTokenIdForClass: NOT_ENOUGH_TOKENS_FOR_CLASS");
  }
   ...
   */
  function _random(BundleRandomnessState storage _state) internal returns (uint256) {
  ...
  function _addTokenIdToClass(BundleRandomnessState storage _state, uint256 _classId, uint256 _tokenId) internal {
